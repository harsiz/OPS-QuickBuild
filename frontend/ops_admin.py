...
MOD_CHOICES = [
    ...
    (64, 'DT/NC'), (256, 'HT'), (1024, 'FL'), (4096, 'SO'), (8192, 'AP'),
]
PP_DEFAULTS = {
    # base multiplier
    'base_min': 1.5, 'base_max': 3.5, 'full_at': 500.0,
    'curve': 'front', 'curve_exp': None,
    # mod rules
    'mod_rules': [],
    # skill skew
    'stream_buff': 50.0, 'aim_nerf': 15.0,
    'speed_neutral': 0.40, 'speed_range': 0.25,
    # accuracy rules (0 = off)
    'acc_bonus_max': 0.0, 'acc_bonus_from': 95.0,
    'acc_penalty': 0.0, 'acc_penalty_below': 85.0,
    # miss punishment (0 = off)
    'miss_penalty': 0.0, 'miss_penalty_cap': 30.0,
    # star rating rules (0 = off)
    'star_bonus_per': 0.0, 'star_bonus_from': 6.0, 'star_bonus_cap': 50.0,
    'low_star_nerf': 0.0, 'low_star_below': 3.0,
    # limits
    'cap': 4000.0, 'cap_mode': 'hard', 'soft_exp': 0.8,
    'relax_factor': 0.35, 'final_mult': 1.0,
}
...
def _generate_profile(cfg: dict, flags: dict) -> str:
    ease = cfg.get('curve_exp') or CURVE_EASE[cfg['curve']]
    L = [
    ...
    if cfg['stream_buff'] or cfg['aim_nerf']:
        L += [
            ...
            f"        skew = max(-1.0, min(1.0, (share - {cfg['speed_neutral']!r}) / {cfg['speed_range']!r}))",
            '        if skew > 0:  # stream/burst map',
            f"            new_pp *= 1.0 + {cfg['stream_buff'] / 100.0!r} * skew",
            '        else:  # aim map (skew is negative)',
            f"            new_pp *= 1.0 + {cfg['aim_nerf'] / 100.0!r} * skew",
        ]

    if cfg['star_bonus_per'] or cfg['low_star_nerf']:
        L += ['    stars = ctx.get("stars")', '    if stars is not None:']
        if cfg['star_bonus_per']:
            L.append(f"        if stars > {cfg['star_bonus_from']!r}:  # star rating bonus")
            L.append(f"            new_pp *= 1.0 + min({cfg['star_bonus_per'] / 100.0!r}"
                     f" * (stars - {cfg['star_bonus_from']!r}), {cfg['star_bonus_cap'] / 100.0!r})")
        if cfg['low_star_nerf']:
            L.append(f"        if stars < {cfg['low_star_below']!r}:  # low-diff farm nerf")
            L.append(f"            new_pp *= {1.0 - cfg['low_star_nerf'] / 100.0!r}")

    if cfg['acc_bonus_max'] or cfg['acc_penalty']:
        L += ['    acc = ctx.get("acc")', '    if acc is not None:']
        if cfg['acc_bonus_max']:
            span = max(100.0 - cfg['acc_bonus_from'], 0.1)
            L.append(f"        if acc >= {cfg['acc_bonus_from']!r}:  # high-acc bonus")
            L.append(f"            new_pp *= 1.0 + {cfg['acc_bonus_max'] / 100.0!r}"
                     f" * min((acc - {cfg['acc_bonus_from']!r}) / {span!r}, 1.0)")
        if cfg['acc_penalty']:
            L.append(f"        if acc < {cfg['acc_penalty_below']!r}:  # low-acc penalty")
            L.append(f"            new_pp *= {1.0 - cfg['acc_penalty'] / 100.0!r}")

    if cfg['miss_penalty']:
        L += [
            '    nmiss = ctx.get("nmiss") or 0',
            '    if nmiss:  # miss punishment',
            f"        new_pp *= max(1.0 - {cfg['miss_penalty'] / 100.0!r} * nmiss,"
            f" {1.0 - cfg['miss_penalty_cap'] / 100.0!r})",
        ]
    ...
    if cfg.get('final_mult') and cfg['final_mult'] != 1.0:
        L.append(f"    new_pp *= {cfg['final_mult']!r}  # final global multiplier")

    if cfg.get('cap'):
        if cfg.get('cap_mode') == 'soft':
            L += [
                f"    if new_pp > {cfg['cap']!r}:  # softcap — diminishing returns past here",
                f"        new_pp = {cfg['cap']!r} + (new_pp - {cfg['cap']!r}) ** {cfg['soft_exp']!r}",
                '    return new_pp',
            ]
        else:
...
async def pp_save():
    ...
    cfg = {
        ...
        'curve_exp': (fnum('curve_exp', 0.0, 0.05, 5.0)
                      if form.get('curve_exp', '').strip() else None),
        'stream_buff': fnum('stream_buff', 0.0, 0.0, 500.0),
        'aim_nerf': fnum('aim_nerf', 0.0, 0.0, 90.0),
        'speed_neutral': fnum('speed_neutral', 0.40, 0.05, 0.95),
        'speed_range': fnum('speed_range', 0.25, 0.05, 0.90),
        'acc_bonus_max': fnum('acc_bonus_max', 0.0, 0.0, 500.0),
        'acc_bonus_from': fnum('acc_bonus_from', 95.0, 50.0, 99.9),
        'acc_penalty': fnum('acc_penalty', 0.0, 0.0, 90.0),
        'acc_penalty_below': fnum('acc_penalty_below', 85.0, 1.0, 100.0),
        'miss_penalty': fnum('miss_penalty', 0.0, 0.0, 50.0),
        'miss_penalty_cap': fnum('miss_penalty_cap', 30.0, 0.0, 95.0),
        'star_bonus_per': fnum('star_bonus_per', 0.0, 0.0, 200.0),
        'star_bonus_from': fnum('star_bonus_from', 6.0, 0.0, 15.0),
        'star_bonus_cap': fnum('star_bonus_cap', 50.0, 0.0, 1000.0),
        'low_star_nerf': fnum('low_star_nerf', 0.0, 0.0, 95.0),
        'low_star_below': fnum('low_star_below', 3.0, 0.0, 15.0),
        'cap': fnum('cap', 0.0, 0.0, 10.0 ** 9) or None,
        'cap_mode': 'soft' if form.get('cap_mode') == 'soft' else 'hard',
        'soft_exp': fnum('soft_exp', 0.8, 0.1, 1.0),
        'relax_factor': fnum('relax_factor', 1.0, 0.0, 5.0),
        'final_mult': fnum('final_mult', 1.0, 0.05, 100.0),
        'mod_rules': [],
    }
